...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const vet = await db("VeterinariasMaestra")
      .select("id")
      .where("id", veterinaria_id)
    ...
    if (!vet) {
      return res.status(400).json({
        success: false,
        message: "La veterinaria seleccionada no está disponible o no ha sido aprobada",
      });
    }
    ...
    const data = {
      ...
      fecha_preferida,
      hora_preferida: hora_preferida && /^\d{2}:\d{2}$/.test(hora_preferida) ? hora_preferida : null,
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    if (status === "confirmed" && !owned.horario_confirmado) {
      return res.status(400).json({
        success: false,
        message: "No puedes confirmar la cita hasta establecer un horario.",
      });
    }

    await db("citas").update({ status, updated_at: new Date() }).where({ id });
...
exports.reschedule = async (req, res) => {
  try {
    ...
    const { fecha_preferida, hora_preferida } = req.body;
    if (!fecha_preferida) {
