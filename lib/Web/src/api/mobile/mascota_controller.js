...
const getPets = async (req, res) => {
    //Obtenemos el IdUsuario del token decodificado (viene del requireAuth)
    const idUsuario = req.user.idUsuario; 
    ...
    try {
        ...
        return res.status(200).json({ 
            success: true, 
            message: 'Lista de mascotas obtenida.',
            //CLAVE: Envía los datos con la clave 'data' que Flutter espera
            data: mascotas 
        });

    } catch (error) {
        console.error('Error al obtener mascotas:', error);
        return res.status(500).json({ success: false, message: 'Error interno al consultar mascotas.' });
    }
};
