  ...
  const enviarActualizacion = async (
    ...
    try {
      const response = await fetch(
        API_ENDPOINTS.veterinarias.actualizarEstado(Number(id)),
        {
          method: "PUT",
      ...
      if (response.ok) {
        ...
        alert(
          `Error: ${errorData.mensaje || "No se pudo actualizar el estado."}`
        );
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span
            style={{ fontWeight: "900", color: "#002D62", fontSize: "18px" }}
          >
            PetCare
          </span>
          <span
            style={{ fontWeight: "bold", color: "#33CCFF", fontSize: "15px" }}
          >
            Manager
          </span>
        </div>
      ...
      <div style={styles.container}>
        <Link to="/admin" style={styles.btnBack}>
          ← Volver al listado
        </Link>
          ...
          <div style={styles.grid}>
            <div style={styles.field}>
              ...
              <div style={styles.value}>
                {data.nombre_responsable} {data.apellidos_responsable}
              </div>
            </div>
            <div style={styles.field}>
              <span style={styles.label}>Puesto</span>
              <div style={styles.value}>{data.puesto}</div>
            </div>
            <div style={styles.field}>
              <span style={styles.label}>Email</span>
              <div style={styles.value}>{data.email_responsable}</div>
            </div>
            <div style={styles.field}>
              <span style={styles.label}>Teléfono</span>
              <div style={styles.value}>{data.telefono_responsable}</div>
            </div>
          </div>
        </div>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.grid}>
            <div style={styles.field}>
              ...
              <div style={styles.value}>{data.razon_social || "N/A"}</div>
            </div>
            <div style={styles.field}>
              <span style={styles.label}>RFC</span>
              <div style={styles.value}>{data.rfc || "N/A"}</div>
            </div>
            <div style={styles.field}>
              <span style={styles.label}>Categorías</span>
              <div style={styles.value}>{data.categorias}</div>
            </div>
            <div style={styles.field}>
              <span style={styles.label}>Descripción</span>
              <div style={styles.value}>{data.descripcion}</div>
            </div>
          </div>
        </div>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.grid}>
            <div style={styles.field}>
              ...
              <div style={styles.value}>
                {data.calle} #{data.numero_exterior}, {data.colonia}
              </div>
            </div>
            <div style={styles.field}>
              <span style={styles.label}>Ubicación</span>
              <div style={styles.value}>
                {data.ciudad}, {data.estado} (CP: {data.codigo_postal})
              </div>
            </div>
            <div style={styles.field}>
              <span style={styles.label}>Teléfono Clínica</span>
              <div style={styles.value}>{data.telefono_clinica}</div>
            </div>
            <div style={styles.field}>
              <span style={styles.label}>Email Clínica</span>
              <div style={styles.value}>{data.email_clinica}</div>
            </div>
          </div>
        </div>
          ...
          <div style={styles.card}>
            ...
            <div>
              <p style={{ fontSize: "14px", color: "#555" }}>
                ...
                {data.horario_apertura || "No especificado"}
              </p>
              <p style={{ fontSize: "14px", color: "#555" }}>
                <strong>Cierre:</strong>{" "}
                {data.horario_cierre || "No especificado"}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden animate-fade-in">
            ...
            <div className="p-6">
              <p className="text-sm text-gray-600 mb-4">
                Por favor, indica el motivo del rechazo. Esta información es
                importante para el solicitante.
              </p>
              ...
              <textarea
                ...
                rows={4}
                placeholder="Ej: La documentación del RFC no es legible..."
