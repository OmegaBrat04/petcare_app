...
interface FormStepProps {
  ...
  handleEnviarVerificacion: () => void; // Función de envío final
}
...
const FormStep: React.FC<FormStepProps> = ({
  ...
  const [newServicePrice, setNewServicePrice] = useState(""); // Función para agregar un nuevo servicio
    ...
    3: (
      ...
      >
        ...
        <div className="mt-4 rounded-2xl overflow-hidden border border-blue-100">
          <div className="px-4 py-2 text-sm text-gray-600 border-b border-blue-100 bg-blue-50">
          ...
          <div className="relative">
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-full">
              ...
              <div className="bg-white/90 text-[10px] px-2 py-1 rounded-md border border-blue-100 mt-1 shadow">
                Nueva sucursal
              </div>
            </div>
          </div>
...
export default function AgregarVeterinariaUI() {
  ...
  const handleEnviarVerificacion = async () => {
    // 🚨 VALIDACIÓN CRÍTICA DEL FRONTEND
    if (
      !formData.nombreResponsable ||
      !formData.nombreComercial ||
      !formData.emailResponsable
    ) {
      alert(
        "Por favor, completa Nombre, Nombre Comercial y Email del responsable. Son campos obligatorios."
      );
      return;
    }
    ...
    try {
      ...
      // 2. HACER LA PETICIÓN FETCH (URL ABSOLUTA para evitar errores de proxy)
      ...
      if (response.ok) {
        alert(`Registro enviado con éxito ✅. ID: ${data.id || "N/A"}`);
      } else {
        alert(
      ...
      alert(
        "No se pudo conectar con el servidor backend. ¿Está corriendo en el puerto 3001?"
      );
    }
  };
          ...
          <div className="card-base p-4 flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-gray-600">
                Puedes enviar a verificación cuando completes los campos.
              </div>
            </div>
            <button className="btn-primary" onClick={handleEnviarVerificacion}>
              Enviar a verificación
